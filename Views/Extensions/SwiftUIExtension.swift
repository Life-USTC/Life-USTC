...
extension View {
    func hStackLeading() -> some View { modifier(HStackModifier()) }

    func hStackTrailing() -> some View {
        modifier(HStackModifier(trailing: true))
    }

    func edgesIgnoringHorizontal(_: Edge.Set) -> some View { self }
    ...
    @ViewBuilder func `if`(_ condition: Bool, transform: (Self) -> some View)
        -> some View
